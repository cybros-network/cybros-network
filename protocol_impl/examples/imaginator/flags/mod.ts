...
>(
  ...
  const argv: Args = { _: [], _1: undefined };
  ...
  const beforeFlags: string[] = [];
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (/^-/.test(arg)) {
      break;
    }
    beforeFlags.push(arg);
  }
  if (beforeFlags.length > 0) {
    argv._1 = beforeFlags.join(" ");
    args = args.slice(beforeFlags.length);
  }
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (/^--.+=/.test(arg)) {
      ...
      if (
        ...
        const values = [];
        while (next !== undefined && !/^-/.test(next)) {
          i++;
          values.push(args[i]);
          next = args[i + 1];
        }
        setArg(key, values.join(" "), arg);
