...
fn transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceId::approve_transfer(
		...
		assert_ok!(DeviceId::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(4)));
...
fn approval_lifecycle_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceId::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(4)));
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		...
		assert_ok!(DeviceId::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(2)));
...
fn cancel_approval_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 1, 42, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 43, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(4)),
			Error::<Test>::NotDelegate
		);

		assert_ok!(DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(3)));
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(3)),
			Error::<Test>::NotDelegate
		);
		...
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(5)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);

		System::set_block_number(current_block + 3);
		// 5 can cancel the approval since the deadline has passed.
		assert_ok!(DeviceId::cancel_approval(RuntimeOrigin::signed(account(5)), 0, 42, account(3)));
...
fn approving_multiple_accounts_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceId::transfer(RuntimeOrigin::signed(account(4)), 0, 42, account(6)));
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(7)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(5)), 0, 42, account(8)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn approvals_limit_works() {
	new_test_ext().execute_with(|| {
		...
		// the limit is 10
		assert_noop!(
			DeviceId::approve_transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(14), None),
...
fn approval_deadline_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(4)),
		...
		assert_ok!(DeviceId::transfer(RuntimeOrigin::signed(account(6)), 0, 42, account(5)));
	});
}
...
fn cancel_approval_works_with_admin() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 1, 42, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 43, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(4)),
		...
		assert_ok!(DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(3)));
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(1)),
			Error::<Test>::NotDelegate
		);
	});
}
...
fn cancel_approval_works_with_force() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::root(), 1, 42, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::root(), 0, 43, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::root(), 0, 42, account(4)),
			Error::<Test>::NotDelegate
		);

		assert_ok!(DeviceId::cancel_approval(RuntimeOrigin::root(), 0, 42, account(3)));
		assert_noop!(
			DeviceId::cancel_approval(RuntimeOrigin::root(), 0, 42, account(1)),
			Error::<Test>::NotDelegate
		);
	});
}
...
fn clear_all_transfer_approvals_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::clear_all_transfer_approvals(RuntimeOrigin::signed(account(3)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(DeviceId::clear_all_transfer_approvals(RuntimeOrigin::signed(account(2)), 0, 42));
		...
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceId::transfer(RuntimeOrigin::signed(account(4)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn buy_item_should_work() {
	new_test_ext().execute_with(|| {
		...
		// can't buy for less
		assert_noop!(
			DeviceId::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_1, 1),
			Error::<Test>::BidTooLow
		);

		// pass the higher price to validate it will still deduct correctly
		assert_ok!(DeviceId::buy_item(
		...
		assert_noop!(
			DeviceId::buy_item(RuntimeOrigin::signed(user_1.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);

		// can't buy when the item is listed for a specific buyer
		assert_noop!(
			DeviceId::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);
		...
		assert_noop!(
			DeviceId::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_3, price_2),
		...
		// ensure we can't buy an item when the collection or an item are frozen
		{
			...
			let buy_item_call = mock::RuntimeCall::DeviceId(crate::Call::<Test>::buy_item {
				collection: collection_id,
				item: item_3,
				bid_price: price_1,
			});
			...
			// lock the transfer
			assert_ok!(DeviceId::lock_item_transfer(
				RuntimeOrigin::signed(user_1.clone()),
				collection_id,
				item_3,
			));
...
fn create_cancel_swap_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceId::create_swap(
			RuntimeOrigin::signed(user_id.clone()),
			collection_id,
			item_1,
			collection_id,
		...
		assert_ok!(DeviceId::cancel_swap(
			RuntimeOrigin::signed(user_id.clone()),
			collection_id,
			item_1
		));
		...
		assert_noop!(
			DeviceId::cancel_swap(RuntimeOrigin::signed(account(2)), collection_id, item_1),
			Error::<Test>::NoPermission
		);
		System::set_block_number(expect_deadline + 1);
		assert_ok!(DeviceId::cancel_swap(RuntimeOrigin::signed(account(2)), collection_id, item_1));
...
fn pallet_level_feature_flags_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceId::buy_item(RuntimeOrigin::signed(user_id.clone()), collection_id, item_id, 1),
			Error::<Test>::MethodDisabled
		);
