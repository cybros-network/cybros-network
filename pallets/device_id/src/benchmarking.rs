...
benchmarks_instance_pallet! {
	...
	buy_item {
		let (collection, seller, _) = create_collection::<T>();
		let (item, ..) = mint_item::<T>(0);
		let buyer: T::AccountId = account("buyer", 0, SEED);
		let buyer_lookup = T::Lookup::unlookup(buyer.clone());
		let price = ItemPrice::<T>::from(0u32);
		let origin = SystemOrigin::Signed(seller.clone()).into();
		DeviceId::<T>::set_price(origin, collection, item, Some(price), Some(buyer_lookup))?;
		T::Currency::make_free_balance_be(&buyer, DepositBalanceOf::<T>::max_value());
	}: _(SystemOrigin::Signed(buyer.clone()), collection, item, price)
	verify {
		assert_last_event::<T>(Event::ItemBought {
			collection,
			item,
			price,
			seller,
	...
	verify {
		if !n.is_zero() {
			assert_last_event::<T>(Event::TipSent {
				collection,
				item,
				sender: caller.clone(),
	...
	create_swap {
		let (collection, caller, _) = create_collection::<T>();
		let (item1, ..) = mint_item::<T>(0);
		let (item2, ..) = mint_item::<T>(1);
		let price = ItemPrice::<T>::from(100u32);
	...
	verify {
		...
		assert_last_event::<T>(Event::SwapCreated {
	...
	cancel_swap {
		...
		let price_with_direction = PriceWithDirection { amount: price, direction: price_direction };
		frame_system::Pallet::<T>::set_block_number(One::one());
		DeviceId::<T>::create_swap(origin, collection, item1, collection, Some(item2), Some(price_with_direction.clone()), duration)?;
	}: _(SystemOrigin::Signed(caller.clone()), collection, item1)
	verify {
		assert_last_event::<T>(Event::SwapCancelled {
			offered_collection: collection,
			offered_item: item1,
			desired_collection: collection,
			desired_item: Some(item2),
	...
	claim_swap {
		...
		frame_system::Pallet::<T>::set_block_number(One::one());
		DeviceId::<T>::transfer(origin.clone().into(), collection, item2, target_lookup)?;
		DeviceId::<T>::create_swap(
			origin.clone().into(),
			collection,
			item1,
			collection,
	...
	verify {
		let current_block = frame_system::Pallet::<T>::block_number();
		assert_last_event::<T>(Event::SwapClaimed {
